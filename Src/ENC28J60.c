...
#define	SRESET  ((0x7<<5) | 0x1F)	// System Reset command does not use an address.
...
void MACInit(void)
{
	...
	// Set up the SPI module on the STM32 for communications with the ENC28J60
#if 0
...
WORD MACGetArray(BYTE *val, WORD len)
{
	...
	//*((__IO uint8_t *)&SPI1->DR)	=	*((uint16_t *)&tx_data[0]);
	//while ((SPI1->SR & SPI_FLAG_FTLVL) != SPI_FTLVL_EMPTY) {}
	//while ((SPI1->SR & SPI_FLAG_BSY) != RESET) {}
    //tmpreg_ovr = SPI1->DR;
    //tmpreg_ovr = SPI1->SR;

	HAL_SPI_TransmitReceive(&hspi1, &tx_data[0], val, len, 100);
#if 0
	i = 0;		
	val--;
	...
	while(i<len)
	{
		...
		if(val)
		{
			val++;
			tx_data[0] = 0;
			HAL_SPI_TransmitReceive(&hspi1, &tx_data[0], &rx_data, 1, 100);
#if 0
...
void MACPutArray(BYTE *val, WORD len)
{
	...
	HAL_SPI_Transmit(&hspi1, val, len, 100);

#if 0
	// Send the data
	while(len)
	{
		tx_data = *val;	// Start sending the byte
...
static REG ReadMACReg(BYTE Address)
{
	...
	tx_data = 0;				// Send a dummy byte
	HAL_SPI_Transmit(&hspi1, &tx_data, 1, 100);


	tx_data = 0;				// Send another dummy byte to receive the register
	HAL_SPI_TransmitReceive(&hspi1, &tx_data, &rx_data, 1, 100);
...
{		
	...
	//HAL_SPI_Transmit(&hspi1, &tx_data[0], 2, 50);
